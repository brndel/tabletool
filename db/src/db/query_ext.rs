...
use chrono::Utc;
use db_core::{
    compile::{CompilerDiagnostics, CompilerMarker, CompletionHint},
    eval_lang::{
...
impl RecordProvider for RecordQuery {
    fn fetch_record(&self, table_name: &str, record: Ulid) -> Option<Cow<'_, [u8]>> {
        let table = self.tx.open_table(TableWithIdDef::new(table_name)).unwrap();

        let result = table.get(&record.0).unwrap()?;

        Some(Cow::Owned(result.value().to_owned()))
    }

    fn iter_table(&self, table_name: &str, mut f: impl FnMut(Ulid, Cow<'_, [u8]>)) {
        let table = self.tx.open_table(TableWithIdDef::new(table_name)).unwrap();

        for record in table.iter().unwrap() {
...
impl Db {
    ...
    //                 kind: db_core::compile::MarkerKind::Error,
    //             },
    //         ));
    ...
    pub fn run_compiled_expr(&self, program: &CompiledProgram) -> Result<Value, DbError> {
        let tx = self.inner.db.begin_read()?;

        let runtime = EvalRuntime::new(RecordQuery { tx });

        let mut store = self.inner.wasm_store.write().unwrap();

        let result = runtime.run_program(&mut store, &program);
